...
if ($query['codigo']==$_SESSION['passwd'])
{
	if (isset($_REQUEST['b']) and intval($query['clan'])==0)
	{
		...
		$clan=mysqli_query($dbconn,"SELECT * FROM `clanes` WHERE `ID`='$req';");
		if (mysqli_num_rows($clan)>0)
		{
		$soli=mysqli_query($dbconn,"SELECT * FROM `solicitud-clan` WHERE `clan`='$req' AND `userid`='$query[ID]'");
			if (mysqli_num_rows($soli)==0)
			{
				mysqli_query($dbconn,
"INSERT INTO `solicitud-clan` (`ID` ,`clan` ,`userid` ,`fecha`)VALUES (NULL , '$req', '$query[ID]', '".time()."');");
		...
		if (intval($query['clan'])!=0)
		{
			...
			if ($query['nick']==$clan['fundador'])
			{
				...
				$alertaspen = mysqli_query($dbconn,"SELECT * FROM `solicitud-clan` WHERE clan='".$query['clan']."'");
				echo '<h2>Solicitudes de ingreso al clan.</h2>';
				while ($alert = mysqli_fetch_array($alertaspen))
				{
					$quiere=mysqli_fetch_array(mysqli_query($dbconn,"SELECT * FROM `pjs` WHERE `ID` = '".$alert['userid']."'"));
					if ($quiere['clan']=="0")
					{
						...
						if(intval($_REQUEST['borrar'])==intval($alert['ID']))
						{
							...
							mysqli_query($dbconn,"DELETE FROM `solicitud-clan` WHERE ID='".intval($_REQUEST['borrar'])."' AND clan='".$query['clan']."'");
						} elseif (intval($_REQUEST['aceptar'])==intval($alert['ID']) and $entran==true) {
							mysqli_query($dbconn,"UPDATE pjs SET clan='".$query['clan']."' WHERE ID='".$alert['userid']."'");
							mysqli_query($dbconn,"UPDATE clanes SET miembros=miembros+1 WHERE ID='".$query['clan']."'");
							mysqli_query($dbconn,"DELETE FROM `solicitud-clan` WHERE userid='".$alert['userid']."'");
							$agregado=true;
						} else {
							if($entran==true){
				...
				$alertaspe = mysqli_query($dbconn,"SELECT * FROM `pjs` WHERE clan='".$query['clan']."'");
				while ($pj = mysqli_fetch_array($alertaspe))
				{

					if($query['ID']!=$pj['ID']){
						if(intval($_REQUEST['echar'])==intval($pj['ID']))
						{
							mysqli_query($dbconn,"UPDATE pjs SET clan='0' WHERE ID='".$pj['ID']."'");
							mysqli_query($dbconn,"UPDATE clanes SET miembros=miembros-1 WHERE ID='".$query['clan']."'");
						}
				...
				{
					mysqli_query($dbconn,"UPDATE clanes SET lvl=lvl+2,puntos=puntos-$necesarios WHERE ID='".$query['clan']."'");
					$clan['lvl']=$clan['lvl']+2;
