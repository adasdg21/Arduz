...
<p>Para jugar no es necesario registrarse en nuestra web http://arduzlegacy.com/index.php, pero si se juega desde un usuario registrado, los beneficios son obvios:
</p><br/>
...
13. Thormut</p><br/>
					...
					<?php
						$result = mysqli_query($dbconn,"SELECT * FROM `configuracion`");
					...
					<?php 
						mysqli_query($_SESSION['dbconn'],"DELETE FROM `servers` WHERE `ultima` < '".(time()-900)."'");
						...
						while ($row=mysqli_fetch_array($result)){
								...
								echo '
							<tr>
								<td><a title="Hora de inicio '.$row['inicio'].'<br/><b>'.$row['players'].' Jugadores</b>" href="#" class="tooltip" title="'.$ii.'">'.htmlspecialchars($row['Nombre']).'</a></td>
